	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// Check if hole should spawn.  Make sure it doesn't spawn under character and give it a random chance to be generated.
				if (!(x != characterController.transform.position.x &&
